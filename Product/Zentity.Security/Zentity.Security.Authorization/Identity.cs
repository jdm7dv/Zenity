...
namespace Zentity.Security.Authorization
{
    ...
    /// <summary>
    /// Represents an entity which accesses zentity core.
    /// </summary>
    public partial class Identity
    {
        /// <summary>
        /// Returns all identities which have given predicates authorization on the resource.
        /// </summary>
        ...
        /// <returns>List of Identity objects with authorization on the given resource.</returns>
        public static IQueryable<Identity> GetAuthorizedIdentities<T>(T resource, IQueryable<string> predicateUris, ZentityContext context)
            where T : Resource
        {
            ...
            IQueryable<Relationship> allAuthorizedRelationships = (context.Relationships.Where(rel =>
                rel.Object.Id == resource.Id
                && rel.Subject is Identity) as ObjectQuery<Relationship>)
                .Include("Predicate")
                .Include("Subject");
            IQueryable<Relationship> authorizedRelationships = from uri in predicateUris
                                                               ...
                                                               select rel;
            IQueryable<Identity> authorizedIdentities = authorizedRelationships.Select(rel => rel.Subject).OfType<Identity>().Distinct();
            return authorizedIdentities;
        }

        /// <summary>
        /// Verfies whether the current identity has a particular permission on resource or not.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// Boolean indicating whether the identity has access or not.
        /// </returns>
        ...
        ///         .Where(tuple => tuple.IdentityName.Equals("User1")).First();
        ///
        ///     Resource resource = context.Resources.Where(tuple => tuple.Title.Equals("Resource1")).First();
        ///
        ///     bool hasAccess = identity.VerifyAuthorization(authorizingPredicate, resource, context));
        /// }
        /// </code>
        /// </example>
        ...
        /// <summary>
        /// Grants permission on resource for the current identity.
        /// </summary>
        /// <typeparam name="T">Zentity.Core.Resource or its derived type</typeparam>
        /// <param name="authorizingPredicateUri">
        /// Predicate corresponding to the permission.
        ...
        ///     Resource resource = context.Resources.Where(tuple => tuple.Title.Equals("Resource1")).First();
        ///
        ///     identity.GrantAuthorization(authorizingPredicate, resource, context);
        ///     context.SaveChanges();
        /// }
        /// </code>
        /// </example>
        ...
        /// <summary>
        /// Revokes permission on resource for the current identity.
        /// </summary>
        /// <typeparam name="T">Zentity.Core.Resource or its derived type</typeparam>
        /// <param name="authorizingPredicateUri">
        /// Predicate corresponding to the permission.
        ...
        ///     Resource resource = context.Resources.Where(tuple => tuple.Title.Equals("Resource1")).First();
        ///
        ///     identity.RevokeAuthorization(authorizingPredicate, resource, context);
        ///     context.SaveChanges();
        /// }
        /// </code>
        /// </example>
        public bool RevokeAuthorization<T>(string authorizingPredicateUri, T resource, ZentityContext context) where T : Resource
        {
            #region Parameter Validation
            ValidateParameters(authorizingPredicateUri, context);
            if (resource == null)
        ...
        /// for which the current identity instance has authorization.</returns>
        ...
        /// for which the identity has authorization.</returns>
