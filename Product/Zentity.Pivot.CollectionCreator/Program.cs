...
using System.Linq;
using System.Text;
using Zentity.CollectionCreator.CommandLine;
using Zentity.CollectionCreator.Commands;
using Zentity.Services.Web;

namespace Zentity.CollectionCreator
{
    /// <summary>
    ...
    static class Program
    {
        ...
        public static int MaxRetries
        {
            get
            {
                ...
                if (!string.IsNullOrWhiteSpace(ConfigurationManager.AppSettings["MaxRetries"]))
                {
                    ...
                    if (retryVal <= 0)
                    {
                        retryVal = Globals.MaxRetryValue;
                    }
                }
                return retryVal;
            }
        ...
        static int Main(string[] commandLineArgs)
        {
            ...
            foreach (string argument in commandLineArgs)
            {
                ...
                {
                    ShowConsoleMessage(string.Format(CultureInfo.InvariantCulture, Properties.Messages.InvalidSwitchWithHelp, argument), true);
                    return (int) ReturnCode.Invalid;
                }
                ...
                {
                    ShowConsoleMessage(string.Format(CultureInfo.InvariantCulture, Properties.Messages.InvalidSwitchAndValuePattern, argument), false);
                    return (int) ReturnCode.Invalid;
                }
                ...
                {
                    ShowConsoleMessage(string.Format(CultureInfo.InvariantCulture, Properties.Messages.InvalidSwitch, arrArgSplit[0].Substring(1)), true);
                    return (int) ReturnCode.Invalid;
                }
            }
        ...
        private static int ProcessCommands(IEnumerable<SwitchCommand> switchCommandList)
        {
            ...
            else if (operationSwitch != null)
            {
                ...
                if (commandDef != null)
                {
                    ...
                    {
                        Console.WriteLine(string.Format(CultureInfo.InvariantCulture, Properties.Messages.ErrorMessage, ex.Message));
                        ShowConsoleMessage(commandDef.GetHelpMessage(), false);
                        return (int) ReturnCode.Error;
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(string.Format(CultureInfo.InvariantCulture, Properties.Messages.ExceptionOccured));
                        Console.WriteLine(string.Format(CultureInfo.InvariantCulture, Properties.Messages.Message, ex.Message));
            ...
            ShowConsoleMessage(string.Format(CultureInfo.InvariantCulture, Properties.Messages.InvalidOperation), true);
            return (int) ReturnCode.Invalid;    
        }
    }
}
