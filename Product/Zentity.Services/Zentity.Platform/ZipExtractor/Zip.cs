 ...
 * Please contact the zslegal alias for assitance.
 * 
...
/*                                   Zip.cs                                  */
/*****************************************************************************/
/* Implements classes that facility reading and writing Zip archives. */
/* Copyright (c) Microsoft Corporation.  All rights reserved. */
/* AUTHOR: Vance Morrison (generic archive file reader/writer--did not understand the zip format)
 *         John Messerly  (added zip format details)
 * Date  : 11/17/2007
 */
/*****************************************************************************/
using System;
...
public sealed class ZipArchive
{
    ...
    public IEnumerable<ZipArchiveFile> GetFilesInDirectory(string archivePath, SearchOption searchOptions)
    {
        foreach (ZipArchiveFile entry in entries.Values)
        {
            ...
            if (name.StartsWith(archivePath, StringComparison.OrdinalIgnoreCase) && name.Length > archivePath.Length)
            {
                ...
                {
                    if (name.IndexOf('\\', archivePath.Length + 1) >= 0)
                        continue;
                    if (name.IndexOf('/', archivePath.Length + 1) >= 0)
                        continue;
                }
                // Some Zips put in zero length files to represent directories (will end with a / or \)
                // filter them out, as they are not files.  
                else if (entry.Length == 0 && name.Length == 0 || name[name.Length - 1] == '\\' || name[name.Length - 1] == '/')
                    continue;
                yield return entry;
            }
        }
    ...
    private static string GetRelativePath(string fileName, string directory)
    {
        ...
        if (directoryEnd == 0)
            return fileName;
        while (directoryEnd < fileName.Length && (fileName[directoryEnd] == '\\' || fileName[directoryEnd] == '/'))
            directoryEnd++;
...
public sealed class ZipArchiveFile
{
    ...
    /// </summary>
    public ZipArchive Archive { get { return archive; } }
    /// <summary>
